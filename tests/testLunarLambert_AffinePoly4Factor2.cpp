...
Vector factorError(const Unit3& s, const Unit3& n, const Pose3& T,
                   const Point3& l, double a, const Vector2& w, const Vector5& c,
                   double lmda, double xi,
                   const LunarLambert_AffinePoly4Factor2 &factor) {
...
/* ************************************************************************* */
TEST(LunarLambert_AffinePoly4Factor2, Jacobians) {
  // Define measurement.
  double meas = 0.1;
  ...
  LunarLambert_AffinePoly4Factor2 factor(sKey, nKey, TKey, lKey, aKey, wKey, 
  ...
  Pose3 T = Pose3(
    Rot3::Random(rng), Point3(get_random(-1, 1), get_random(-1, 1), get_random(-1, 1)));
  Point3 l(get_random(-1, 1), get_random(-1, 1), get_random(-1, 1));
  double a = 0.5;
  double w0 = 0.896;
  ...
  double c0 = 0.301;
  double c1 = -5.17e-3 * 180. / M_PI;
  double c2 = 5.51e-5 * pow(180. / M_PI, 2);
  double c3 = -3.13e-7 * pow(180. / M_PI, 3);
  double c4 = 0.699e-9 * pow(180. / M_PI, 4);
  Vector5 c(c0, c1, c2, c3, c4);
  ...
  Matrix numericalH7 = numericalDerivative11<Vector, Vector5>(
...
// 
//   // Create the factor.
//   Key sKey(0), nKey(1), TKey(2), lKey(3), aKey(4), wKey(5), cKey(6), lmdaKey(7),
//       xiKey(8);
//   SharedNoiseModel model = noiseModel::Isotropic::Sigma(1, 0.5);
//   LunarLambert_AffinePoly4Factor2 factor(sKey, nKey, TKey, lKey, aKey, wKey,
//                                          cKey, lmdaKey, xiKey, meas, model);
...
//   Values vals;
//   vals.insert(sKey,
//               Unit3(get_random(-1, 1), get_random(-1, 1), get_random(-1, 1)));
//   vals.insert(nKey,
//               Unit3(get_random(-1, 1), get_random(-1, 1), get_random(-1, 1)));
//   vals.insert(TKey, Pose3(Rot3(), Point3(get_random(-1, 1), get_random(-1, 1),
//                                          get_random(-1, 1))));
//   vals.insert(lKey,
//               Point3(get_random(-1, 1), get_random(-1, 1), get_random(-1, 1)));
//   vals.insert(aKey, double(0.5));
// 
//   double w0 = 0.830;
//   double w1 = -7.22e-3 * 180. / M_PI;
//   vals.insert(wKey, Vector2(w0, w1));
// 
//   double c0 = 0.301;
//   double c1 = -5.17e-3 * 180. / M_PI;
//   double c2 = 5.51e-5 * pow(180. / M_PI, 2);
//   double c3 = -3.13e-7 * pow(180. / M_PI, 3);
//   double c4 = 0.699e-9 * pow(180. / M_PI, 4);
//   vals.insert(cKey, Vector5(c0, c1, c2, c3, c4));
// 
//   vals.insert(lmdaKey, double(0.8));
//   vals.insert(xiKey, double(0.1));
// 
//   // Compare!
//   EXPECT(
//       gtsam::internal::testFactorJacobians("dummy", factor, vals, 1e-5, 1e-8))
// }
